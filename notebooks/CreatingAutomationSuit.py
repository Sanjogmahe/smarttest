...
#Generate the prioritsed test case list based on relevant condition.
    ...
    if rel == current_release:
        ...
        if t_pri == 'P0' or t_pri == 'P1' and n_def >= 0 and e_p_test == 'Yes' and s_def in ('High', 'Medium'):
            current_release_P0_P1.append((t_id, tc_title, a_script_name, t_pri, n_def, s_def))
        if t_pri == 'P2' or t_pri == 'P3' and n_def >= 2 and e_p_test == 'Yes' and s_def == 'High':
            current_release_P2_P3.append((t_id, tc_title, a_script_name, t_pri, n_def, s_def))

##Process the data for remaining releases and generate the prioritised test case list

    if rel != current_release:
        if t_pri == 'P0' or t_pri == 'P1' and n_def >= 0 and e_p_test == 'Yes' and s_def in ('High', 'Medium'):
            previous_releases_P0_P1.append((t_id, tc_title, a_script_name, t_pri, n_def, s_def))
        if t_pri == 'P2' or t_pri == 'P3' and n_def >= 2 and e_p_test == 'Yes' and s_def == 'High':
            previous_releases_P2_P3.append((t_id, tc_title, a_script_name, t_pri, n_def, s_def))
...
if limit ==0:
    ...
    limited_test = pd.DataFrame(all_info_tc, columns=["Test ID", "Test case Title", "Automation Script Name","Test Priority",
                                               "Linked Defects", "Severity Defects"])
    ...
    for i in range(len(master["Test ID"])):
        final.append((master["Test ID"][i], master["Test case Title"][i],master["Automation Script Name"][i]))
    master_dataframe = pd.DataFrame(final, columns=["Test ID", "Test case Title", "Automation Script Name"])
    master_dataframe = master_dataframe.head(limit)
    master_dataframe.to_excel(writer, sheet_name="Master_Output",index=False)
    # master_dataframe.set_index(master_dataframe['index'].head(limit), inplace=True)
# writer.save()
...
if limit == 0:
    ...
    for i in range(len(df['Test ID'])):
        if df['Automation Script Name'][i] == "Manual Testing":
            m_tc.append((df['Test ID'][i], df['Test case Title'][i]))
        else:
            a_tc.append(df['Automation Script Name'][i])

df_auto = pd.DataFrame(set(a_tc), columns=["Automation Script Name"])
df_auto.index += 1
df_auto.index.name="S.No."
df_auto.to_excel(writer, sheet_name="Automation_Scripts")
df_manual = pd.DataFrame(m_tc, columns=["Test ID", "Manual Test Case"])
df_manual.index += 1
